...
# #             nn.Flatten(),                       # (B, C)
# #             nn.Linear(dim // num_heads, dim // num_heads),          # produce one gate per head
...
# #                 gate = self.gate_fc(feat)            # (B, num_heads)

# #                 # 取当前 head 的 gate_i，reshape 成 (B, 1, 1, 1)     方便广播
# #                 gate_i = gate.view(B, -1, 1, 1)
    
# #                 # gated residual input
# #                 feat = feat + gate_i * feats_in[i]
...
#         self.d = int(attn_ratio * key_dim)
#         self.attn_ratio = attn_ratio
#         gate_hidden = max(dim // 16, 16)
#         self.gate_threshold = 0.15
#         self.gate_fc = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(dim, gate_hidden, 1, bias=True),
#             nn.ReLU(inplace=True),
#             nn.Conv2d(gate_hidden, num_heads, 1, bias=True),
...
#         B, C, H, W = x.shape
#         trainingab = self.attention_biases[:, self.attention_bias_idxs]
#         head_gates = self.gate_fc(x)
#         feats_in = x.chunk(len(self.qkvs), dim=1)
#         feats_out = []
#         feat = feats_in[0]
#         for i, qkv in enumerate(self.qkvs):
#             gate_i = head_gates[:, i:i + 1]
#             if i > 0:
#                 feat = feat + gate_i * feats_in[i]
#                 if not self.training and gate_i.mean().item() < self.gate_threshold:
#                     feats_out.append(feat.new_zeros(B, self.d, H, W))
#                     continue
#             feat = qkv(feat)
...
#     def gaussian_kernel_1d(self, channels, kernel_size=None, sigma=None, device=None, dtype=None):
#         kernel_size = min(kernel_size or self.freq_kernel_size, channels)
#         sigma = sigma or self.freq_sigma
#         coords = torch.arange(channels, device=device, dtype=dtype) - (channels - 1) / 2
#         radius = max((kernel_size - 1) / 2, 1)
#         kernel = torch.exp(-0.5 * (coords / (sigma * radius)) ** 2)
#         return (kernel / kernel.max()).view(1, 1, channels)

#     def frequency_token_select(self, x):
#         if not self.freq_select:
#             return x

#         b, c, h, w = x.shape
#         tokens = x.flatten(2).transpose(1, 2)
#         tokens_detach = tokens
#         fft_tokens = tokens.float() if tokens.dtype in (torch.float16, torch.bfloat16) else tokens
#         freq = torch.fft.fft(fft_tokens, dim=-1)
#         gs_k = self.gaussian_kernel_1d(c, device=x.device, dtype=fft_tokens.dtype)
#         freq = torch.fft.fftshift(freq, dim=-1)
#         freq = freq * gs_k
#         freq = torch.fft.ifftshift(freq, dim=-1)
#         filtered = torch.fft.ifft(freq, dim=-1).real.to(tokens.dtype)
#         diff = tokens_detach / (torch.abs(filtered - tokens_detach) + self.freq_eps)
#         _, indices = torch.topk(diff, k=1, dim=1, largest=True)
#         sel_p = torch.gather(tokens_detach, 1, indices)
#         cls_token = torch.mean(sel_p, dim=1)
#         gate = torch.sigmoid(cls_token).view(b, c, 1, 1)
#         return x * gate
...
#         x = self.patch_embed(x)
#         x = self.blocks1(x)
#         x = self.frequency_token_select(x)
#         outs.append(x)
#         x = self.blocks2(x)
#         x = self.frequency_token_select(x)
#         outs.append(x)
...
class EfficientViT(torch.nn.Module):
    def __init__(self, img_size=400,
        ...
        self.freq_stages = set(freq_stages)
    ...
    def gaussian_kernel_1d(self, length, kernel_size=None, sigma=None, device=None, dtype=None):
        kernel_size = min(kernel_size or self.freq_kernel_size, length)
        sigma = sigma or self.freq_sigma
        coords = torch.arange(length, device=device, dtype=dtype) - (length - 1) / 2
        radius = max((kernel_size - 1) / 2, 1)
        kernel = torch.exp(-0.5 * (coords / (sigma * radius)) ** 2)
        return (kernel / kernel.max()).view(1, 1, length)

    def frequency_token_select(self, x, stage_idx):
        if not self.freq_select:
            return x
        if stage_idx not in self.freq_stages:
            return x
        ...
        fft_tokens = tokens.float() if tokens.dtype in (torch.float16, torch.bfloat16) else tokens
        freq = torch.fft.fft(fft_tokens, dim=-1)
        gs_k = self.gaussian_kernel_1d(n, device=x.device, dtype=fft_tokens.dtype)
        freq = torch.fft.fftshift(freq, dim=-1)
        freq = freq * (1.0 - gs_k)
        freq = torch.fft.ifftshift(freq, dim=-1)
        high_freq = torch.fft.ifft(freq, dim=-1).real.to(tokens.dtype).view(b, c, h, w)
        energy = high_freq.abs().mean(dim=(2, 3), keepdim=True)
        gate = torch.sigmoid(energy)
        return x + self.freq_alpha * high_freq * gate

    def forward(self, x):
        outs = []
        x = self.patch_embed(x)
        x = self.blocks1(x)
        x = self.frequency_token_select(x, stage_idx=0)
        outs.append(x)
        x = self.blocks2(x)
        x = self.frequency_token_select(x, stage_idx=1)
        outs.append(x)
        x = self.blocks3(x)
        x = self.frequency_token_select(x, stage_idx=2)
        outs.append(x)
        return outs
